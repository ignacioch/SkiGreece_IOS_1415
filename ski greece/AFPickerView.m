...
- (void)hidePicker {
    ...
    selectedRow = position;
    [contentView setContentOffset:CGPointMake(0.0, ROW_SPACE * position + CONTENT_OFFSET_Y) animated:YES];
    [delegate pickerView:self hidedRow:selectedRow];
    
    NSLog(@"Y: %f Height: %f Center:%f", self.frame.origin.y, self.frame.size.height,ROW_SPACE * position + CONTENT_OFFSET_Y);
...
- (UIToolbar *)toolbar:(NSString *)title {
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f) {
        toolbar.barTintColor = RGBACOLOR(254, 172, 64, 1);
    } else {
